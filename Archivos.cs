...
public static class Archivos{
    ...
    public static void LeerCadetes(string nombre, List<Cadetes> Cadts){
        using(TextFieldParser ruta = new TextFieldParser(nombre+".csv")){
            ruta.TextFieldType = FieldType.Delimited;
            ruta.SetDelimiters(",",";");
            while(!ruta.EndOfData){
                ...
                if(filas.Count()==4){
                    ...
                    var cad = new Cadetes(id,nom,dire,tel);
                    Cadts.Add(cad);
                }else{
                    System.Console.WriteLine("no tiene el formato adecuado");
                }
    ...
    public static void GuardarResumen(Cadeteria Cdtria){
        try{
            ...
            using (StreamWriter arch = new StreamWriter("Resumen.txt")){
                ...
                foreach (var c in Cdtria.Cadeteros){
                    var id = c.Id;
                    var nombre = c.Nombre;
                    var pE = c.CantidadDePedidos(1);
                    var pSE = c.CantidadDePedidos(2);
                    var pC = c.CantidadDePedidos(3);
                    var TP = c.CantidadDePedidos(0);
                    var pago = c.JornalACobrar();
                    arch.WriteLine(id+"| "+nombre+", PE"+pE+" PSE:"+pSE+" PC:"+pC+" PT:"+TP+", JORNAL: "+pago);
                }
                var numeroPed = Cdtria.NumeradorPedidos-1; 
